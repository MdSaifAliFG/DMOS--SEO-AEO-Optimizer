    ...
    <DashboardShell>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800 shadow-xs">
          <div className="space-y-1">
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">SEO Optimization History</h1>
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800 rounded-2xl p-12 text-center max-w-lg mx-auto shadow-xs">
            <History className="w-12 h-12 text-slate-400 dark:text-slate-500 mx-auto mb-3" />
            <h3 className="text-base font-semibold text-slate-800 dark:text-white">No Comparison History Yet</h3>
          ...
          <div className="space-y-4">
            {history.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 space-y-4 shadow-xs hover:border-slate-300 dark:hover:border-slate-700 transition"
                >
                  {/* Comparison Header */}
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-3.5">
                    <div className="flex items-center gap-3">
                      <div className="p-2.5 rounded-xl bg-sky-50 dark:bg-sky-950/40 text-sky-600 dark:text-sky-400 border border-sky-100 dark:border-sky-800">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <div className="text-xs text-slate-500 dark:text-slate-400">Score Evolution</div>
                        <div className="text-sm font-bold text-slate-700 dark:text-slate-300">
                          {item.previous_score !== null ? `${item.previous_score} pts` : "Initial"} →{" "}
                          <span className="text-slate-900 dark:text-white">{item.current_score} pts</span>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    <div className="bg-slate-50/80 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="text-[11px] text-emerald-700 dark:text-emerald-400 font-semibold flex items-center gap-1">
                        <CheckCircle2 className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> Issues Resolved
                      </div>
                      <div className="text-xl font-bold text-slate-900 dark:text-white mt-1">{item.issues_resolved}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400">Fixed since last scan</div>
                    </div>

                    <div className="bg-slate-50/80 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="text-[11px] text-rose-700 dark:text-rose-400 font-semibold flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3 text-rose-600 dark:text-rose-400" /> New Issues
                      </div>
                      <div className="text-xl font-bold text-slate-900 dark:text-white mt-1">{item.new_issues}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400">Newly detected</div>
                    </div>

                    <div className="bg-slate-50/80 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="text-[11px] text-amber-700 dark:text-amber-400 font-semibold">Remaining Issues</div>
                      <div className="text-xl font-bold text-slate-900 dark:text-white mt-1">{item.remaining_issues}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400">Still requiring fixes</div>
                    </div>

                    <div className="bg-slate-50/80 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="text-[11px] text-sky-700 dark:text-sky-400 font-semibold">Pages Improved</div>
                      <div className="text-xl font-bold text-slate-900 dark:text-white mt-1">{item.pages_improved}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400">Higher page score</div>
                    </div>

                    <div className="bg-slate-50/80 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                      <div className="text-[11px] text-slate-700 dark:text-slate-300 font-semibold">Pages Declined</div>
                      <div className="text-xl font-bold text-slate-900 dark:text-white mt-1">{item.pages_declined}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400">Lower page score</div>
                    </div>
                  </div>
                  ...
                  {Object.keys(item.category_score_changes || {}).length > 0 && (
                    <div className="flex flex-wrap items-center gap-2 pt-1 text-xs">
                      <span className="text-slate-500 dark:text-slate-400 font-medium">Category Changes:</span>
                      {Object.entries(item.category_score_changes).map(([cat, delta]) => (
                        <span
                          key={cat}
                          className={cn(
                            "px-2 py-0.5 rounded-md font-mono font-medium border",
                            delta > 0 && "bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800",
                            delta < 0 && "bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-800",
                            delta === 0 && "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700"
                          )}
                        >
