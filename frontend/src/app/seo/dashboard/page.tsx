              ...
              <div className="flex items-center gap-3">
                <div className="bg-purple-50/80 border border-purple-200 rounded-xl px-3.5 py-2 text-right">
                  ...
                  <div className="text-sm font-bold text-slate-900 mt-0.5">
                    +{optSummary.estimated_seo_impact} pts <span className="text-xs font-medium text-slate-500">(Potential: {optSummary.potential_seo_score}/100)</span>
                  </div>
                </div>
              ...
              {optSummary.category_breakdown.map((cat) => (
                <div key={cat.category} className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-emerald-600 font-bold">{cat.progress_percentage}%</span>
                  </div>
            ...
            {optSummary.top_opportunities.length > 0 && (
              <div className="space-y-2.5 pt-2 border-t border-slate-100">
                <div className="text-xs font-semibold text-slate-700 flex items-center justify-between">
                  ...
                  <Link href="/seo/actions" className="text-emerald-600 hover:text-emerald-700 text-xs font-semibold flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {optSummary.top_opportunities.slice(0, 3).map((opp) => (
                    <Link
                      key={opp.id}
                      href="/seo/actions"
                      className="p-3 bg-slate-50 hover:bg-slate-100/80 rounded-xl border border-slate-200 hover:border-emerald-300 transition-all block group"
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-xs font-bold text-purple-700">+{opp.estimated_impact} pts</span>
                      </div>
                      <div className="text-xs font-bold text-slate-900 group-hover:text-blue-600 transition-colors mt-2 line-clamp-1">
                        {opp.title}
                      </div>
                      <div className="text-[11px] text-slate-500 mt-0.5 line-clamp-1">{opp.description}</div>
                    </Link>
                  ))}
                </div>
              </div>
