      ...
      <DashboardShell>
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <Skeleton key={i} className="h-28 w-full rounded-xl bg-slate-200 dark:bg-slate-800" />
            ))}
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Skeleton className="h-80 lg:col-span-2 rounded-xl bg-slate-200 dark:bg-slate-800" />
    ...
    <DashboardShell>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#0f172a] border border-slate-200/80 dark:border-slate-800 shadow-xs">
          <div className="space-y-1">
            <div className="flex items-center gap-2.5 flex-wrap">
              <div className="p-2 rounded-xl bg-blue-50 border border-blue-100 text-blue-600 dark:bg-blue-950/60 dark:border-blue-800/60 dark:text-blue-400">
                <Globe className="w-5 h-5" />
              </div>
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">SEO Optimization & Auditing</h1>
              <span className="px-2.5 py-0.5 text-xs font-semibold rounded-full bg-blue-50 text-blue-700 border border-blue-200 dark:bg-blue-950/60 dark:text-blue-400 dark:border-blue-800/60">
                Core Engine
              </span>
            </div>
          ...
          <div className="flex items-center gap-2.5 flex-wrap sm:shrink-0">
            <button
              ...
              className="px-3.5 py-2 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 text-xs font-semibold text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 transition-colors flex items-center gap-1.5 shadow-xs"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          {/* Left 2 Cols: SEO Score Trend */}
          <Card className="lg:col-span-2 p-4 sm:p-5 border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">SEO Score Trend</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Historical performance across audit cycles</p>
              </div>
              <Link href="/seo/reports">
                <span className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 flex items-center gap-1 cursor-pointer">
            ...
            {summary?.score_trend && summary.score_trend.length > 0 ? (
              <div className="h-56 flex items-end justify-between gap-3 pt-6 pb-2 px-4 border border-slate-100 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-800/40 overflow-x-auto">
                {summary.score_trend.map((point, idx) => (
                  <div key={idx} className="flex-1 min-w-[36px] flex flex-col items-center gap-2 h-full justify-end">
                    <span className="text-[11px] font-bold font-mono text-slate-700 dark:text-slate-300">{point.score}</span>
                    <div
                      className="w-full max-w-[36px] bg-blue-600 dark:bg-blue-500 rounded-t-md transition-all duration-500 hover:bg-blue-700 dark:hover:bg-blue-400"
                      style={{ height: `${Math.max(15, (point.score / 100) * 160)}px` }}
                    />
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate">{point.date}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="h-56 flex flex-col items-center justify-center p-6 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-800/30">
                <Globe className="w-8 h-8 text-slate-400 mb-2" />
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">No scan history recorded yet</p>
          ...
          <Card className="p-5 border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Crawl Overview</h3>
            ...
            <div className="space-y-3 pt-2">
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700 dark:text-slate-300 font-medium">Crawled & Audited</span>
                </div>
              ...
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700 dark:text-slate-300 font-medium">Discovered Links</span>
                </div>
                <span className="font-bold font-mono text-slate-900 dark:text-white">
              ...
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700 dark:text-slate-300 font-medium">Skipped (Robots.txt)</span>
                </div>
                <span className="font-bold font-mono text-slate-900 dark:text-white">
              ...
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-700 dark:text-slate-300 font-medium">Failed / Errors</span>
                </div>
                <span className="font-bold font-mono text-slate-900 dark:text-white">
        ...
        {optSummary && optSummary.total_actions > 0 && (
          <Card className="p-4 sm:p-6 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] shadow-xs rounded-2xl space-y-5 sm:space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-5">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-800/60 flex items-center gap-1.5">
                    <Sparkles className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> Phase 4 Optimization
                  </span>
              ...
              <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
                <div className="bg-purple-50/80 dark:bg-purple-950/50 border border-purple-200 dark:border-purple-800/60 rounded-xl px-3.5 py-2 text-right">
                  <div className="text-[11px] text-purple-700 dark:text-purple-300 font-semibold flex items-center gap-1 justify-end">
                    <TrendingUp className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" /> Recoverable Impact
                  </div>
                  <div className="text-sm font-bold text-slate-900 dark:text-white mt-0.5">
                    +{optSummary.estimated_seo_impact} pts <span className="text-xs font-medium text-slate-500 dark:text-slate-400">(Potential: {optSummary.potential_seo_score}/100)</span>
                  </div>
                </div>

                <Link
                  href="/seo/actions"
                  className="px-4 py-2 text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition shadow-xs flex items-center gap-1.5"
                >
                  View Action Center →
                </Link>
              </div>
            ...
            <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              {optSummary.category_breakdown.map((cat) => (
                <div key={cat.category} className="bg-slate-50 dark:bg-slate-800/60 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{cat.category}</span>
                    <span className="text-emerald-600 dark:text-emerald-400 font-bold">{cat.progress_percentage}%</span>
                  </div>
            ...
            {optSummary.top_opportunities.length > 0 && (
              <div className="space-y-2.5 pt-2 border-t border-slate-100 dark:border-slate-800">
                <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                  ...
                  <Link href="/seo/actions" className="text-emerald-600 dark:text-emerald-400 hover:underline text-xs font-semibold flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {optSummary.top_opportunities.slice(0, 3).map((opp) => (
                    <Link
                      key={opp.id}
                      href="/seo/actions"
                      className="p-3 bg-slate-50 hover:bg-slate-100/80 dark:bg-slate-800/60 dark:hover:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-500/50 transition-all block group"
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-xs font-bold text-purple-700 dark:text-purple-400">+{opp.estimated_impact} pts</span>
                      </div>
                      <div className="text-xs font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mt-2 line-clamp-1">
                        {opp.title}
                      </div>
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{opp.description}</div>
                    </Link>
                  ))}
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <Card className="p-4 sm:p-5 border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Top Detected Issues</h3>
            ...
            {summary?.top_issues && summary.top_issues.length > 0 ? (
              <div className="space-y-2.5">
                {summary.top_issues.map((issue, idx) => (
                  <Link
                    key={idx}
                    href="/seo/issues"
                    className="p-3 rounded-xl bg-slate-50 hover:bg-slate-100/80 dark:bg-slate-800/60 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-between transition-colors block"
                  >
              ...
              <div className="p-8 text-center bg-slate-50/50 dark:bg-slate-800/30 rounded-xl border border-dashed border-slate-200 dark:border-slate-800">
                <CheckCircle2 className="w-6 h-6 text-emerald-500 mx-auto mb-1.5" />
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">No Critical SEO Issues</p>
          ...
          <Card className="p-4 sm:p-5 border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Monitored Websites</h3>
              <Link href="/seo/projects">
                <span className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 flex items-center gap-1 cursor-pointer">
                ...
                {projects.map((p) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 font-mono flex items-center gap-1 mt-0.5 truncate">
        ...
        {isScoreModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
            <div
              className="relative w-full max-w-lg bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-5 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto"
                ...
                <div className="grid grid-cols-2 gap-2.5 font-medium">
                  <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                    <div className="flex items-center justify-between text-slate-900 dark:text-white font-bold">
                      ...
                      <span className="text-blue-600 dark:text-blue-400 font-mono">30%</span>
                    </div>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">HTTP status, SSL/HTTPS, server response time, mixed content</p>
                  </div>

                  <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                    <div className="flex items-center justify-between text-slate-900 dark:text-white font-bold">
                      <span>Indexability</span>
                      <span className="text-blue-600 dark:text-blue-400 font-mono">25%</span>
                    </div>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Robots.txt, meta robots, canonical URLs, XML sitemaps</p>
                  </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Titles, descriptions, H1 tags, thin content, duplicate content</p>
                  </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Broken links, redirect loops, orphan pages, crawl depth</p>
                  </div>
                </div>

                {/* Score Rating Tiers */}
                <div className="p-3 bg-blue-50/60 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-800/60 rounded-xl space-y-1.5">
                  <span className="font-bold text-slate-900 dark:text-white block text-xs">Rating Tiers:</span>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 text-[11px]">
                    <span className="text-emerald-700 dark:text-emerald-300"><strong>90–100:</strong> Excellent</span>
                    <span className="text-blue-700 dark:text-blue-300"><strong>75–89:</strong> Good</span>
                    <span className="text-amber-700 dark:text-amber-300"><strong>50–74:</strong> Needs Fixes</span>
                    <span className="text-orange-700 dark:text-orange-300"><strong>25–49:</strong> Poor</span>
                    <span className="text-rose-700 dark:text-rose-300"><strong>0–24:</strong> Critical</span>
                  </div>
                </div>
