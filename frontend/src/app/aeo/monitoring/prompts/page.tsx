      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-5">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Total Tracked Prompts</span>
            <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mt-1">{movements.length}</p>
          </div>
          ...
          <div className="bg-emerald-50/60 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/40 rounded-2xl p-5 shadow-xs">
            <span className="text-xs font-bold text-emerald-800 dark:text-emerald-300 flex items-center gap-1 uppercase tracking-wider">
              <TrendingUp className="w-3.5 h-3.5" /> Gained Mentions
            </span>
            <p className="text-2xl sm:text-3xl font-black text-emerald-950 dark:text-emerald-100 mt-1">{gainedCount}</p>
          </div>
          ...
          <div className="bg-rose-50/60 dark:bg-rose-950/40 border border-rose-200/80 dark:border-rose-800/40 rounded-2xl p-5 shadow-xs">
            <span className="text-xs font-bold text-rose-800 dark:text-rose-300 flex items-center gap-1 uppercase tracking-wider">
              <TrendingDown className="w-3.5 h-3.5" /> Lost Mentions
            </span>
            <p className="text-2xl sm:text-3xl font-black text-rose-950 dark:text-rose-100 mt-1">{lostCount}</p>
          </div>
          ...
          <div className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-5">
            ...
            <p className="text-2xl sm:text-3xl font-black text-slate-700 dark:text-slate-300 mt-1">{unchangedCount}</p>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="p-16 flex flex-col items-center justify-center text-center bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
            ...
            <p className="text-slate-800 dark:text-white font-semibold text-sm">Loading Prompt Trajectories...</p>
          </div>
        ) : projects.length === 0 ? (
          ...
          <Card className="bg-white dark:bg-[#0f172a] border-slate-200/80 dark:border-slate-800 shadow-xs rounded-2xl p-12 text-center">
            <MessageSquare className="w-12 h-12 text-purple-400 mx-auto mb-3" />
            <h3 className="text-base font-bold text-slate-900 dark:text-white">No Prompt Movements Recorded</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-1">
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                  {filteredMovements.map((item) => (
                    <tr key={item.question_id} className="hover:bg-purple-50/20 dark:hover:bg-purple-950/20 transition-colors">
                      <td className="py-3.5 px-4 max-w-md">
                        <span className="font-bold text-slate-900 dark:text-white block truncate">{item.prompt}</span>
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 capitalize">{item.intent}</span>
                        </div>
                      </td>
                      <td className="py-3.5 px-4">
                        <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800/60 uppercase font-bold">
                          {item.provider}
                        </span>
                      </td>
                      <td className="py-3.5 px-4">
                        {item.movement === "gained" ? (
                          <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60 flex items-center gap-1 w-fit">
                            <TrendingUp className="w-3 h-3" /> Gained
                          </span>
                        ) : item.movement === "lost" ? (
                          <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-rose-50 dark:bg-rose-950/60 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-800/60 flex items-center gap-1 w-fit">
                            <TrendingDown className="w-3 h-3" /> Lost
                          </span>
                        ) : (
                          <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 flex items-center gap-1 w-fit">
                            <Minus className="w-3 h-3" /> Unchanged
                          </span>
                        )}
                      </td>
                      <td className="py-3.5 px-4">
                        <span className="font-black text-slate-900 dark:text-white">{item.visibility_score}</span>
                      ...
                      <td className="py-3.5 px-4">
                        {item.citation_found ? (
                          ...
                          <span className="text-slate-400 dark:text-slate-500 font-medium">Not cited</span>
                        )}
                      </td>
                      <td className="py-3.5 px-4">
                        <Link href={`/aeo/optimize/content?question=${encodeURIComponent(item.prompt)}`}>
                          <Button size="sm" variant="secondary" className="border-purple-200 dark:border-purple-800/60 text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/50 hover:bg-purple-100 dark:hover:bg-purple-900/50 text-xs font-semibold rounded-lg px-2.5 py-1">
                            Optimize
                          </Button>
                        </Link>
                      </td>
