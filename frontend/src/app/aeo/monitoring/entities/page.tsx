      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-5">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Tracked Entities</span>
            <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mt-1">{entities.length}</p>
            <span className="text-[11px] text-slate-400 dark:text-slate-500 mt-1 block">Extracted from AI answers</span>
          </div>

          <div className="bg-gradient-to-br from-purple-50 to-indigo-50/50 dark:from-purple-950/40 dark:to-indigo-950/40 border border-purple-200/80 dark:border-purple-800/40 rounded-2xl p-5 shadow-xs">
            <span className="text-xs font-bold text-purple-800 dark:text-purple-300 uppercase tracking-wider block">Brand Entities</span>
            <p className="text-2xl sm:text-3xl font-black text-purple-950 dark:text-white mt-1">{brandEntitiesCount}</p>
            <span className="text-[11px] text-purple-600 dark:text-purple-400 font-semibold mt-1 block">Core brand associations</span>
          </div>

          <div className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-5">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Avg Knowledge Confidence</span>
            <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mt-1">{avgConfidence.toFixed(0)}%</p>
            <span className="text-[11px] text-slate-400 dark:text-slate-500 mt-1 block">Extraction certainty index</span>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="p-16 flex flex-col items-center justify-center text-center bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
            ...
            <p className="text-slate-800 dark:text-white font-semibold text-sm">Inspecting Knowledge Graph Nodes...</p>
          </div>
        ) : projects.length === 0 ? (
          ...
          <Card className="bg-white dark:bg-[#0f172a] border-slate-200/80 dark:border-slate-800 shadow-xs rounded-2xl p-12 text-center">
            <Layers className="w-12 h-12 text-purple-400 mx-auto mb-3" />
            <h3 className="text-base font-bold text-slate-900 dark:text-white">No Entity Movements Found</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredEntities.map((item) => (
              <div
                ...
                className="bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-5 hover:border-purple-300 dark:hover:border-purple-800/60 transition-all flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h3 className="font-bold text-slate-900 dark:text-white text-base">{item.name}</h3>
                      <span className="text-xs text-purple-600 dark:text-purple-400 font-semibold capitalize">{item.entity_type}</span>
                    </div>
                    <span
                  ...
                  <div className="mt-4 space-y-2 text-xs">
                    <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800">
                      <span className="text-slate-500 dark:text-slate-400">Mention Frequency</span>
                      <span className="font-bold text-slate-800 dark:text-slate-200">{item.frequency} times</span>
                    </div>
                    <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {item.associated_concepts.map((concept, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-0.5 rounded-md text-[11px] bg-purple-50 dark:bg-purple-950/60 text-purple-800 dark:text-purple-300 border border-purple-200/80 dark:border-purple-800/60 font-medium flex items-center gap-1"
                          >
                            <Tag className="w-2.5 h-2.5 text-purple-600 dark:text-purple-400" />
                            {concept}
                          </span>
                        ))}
                      </div>
