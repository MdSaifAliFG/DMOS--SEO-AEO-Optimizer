    ...
    <DashboardShell>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
          <Link href="/aeo/projects" className="hover:text-purple-600 dark:hover:text-purple-400 transition-colors">
            AEO Projects
          </Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="p-5 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] flex flex-col justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">AEO Visibility</span>
          ...
          <Card className="p-5 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] flex flex-col justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Mentions (35%)</span>
            <div className="mt-2">
          ...
          <Card className="p-5 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] flex flex-col justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Citations (25%)</span>
            <div className="mt-2">
              <span className="text-3xl font-black text-slate-800 dark:text-slate-100">
          ...
          <Card className="p-5 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] flex flex-col justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Coverage (20%)</span>
            <div className="mt-2">
              <span className="text-3xl font-black text-slate-800 dark:text-slate-100">
        ...
        {/* Main Content Tabs */}
        <Card className="p-6 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <button
                ...
                className={`text-xs font-bold pb-1 border-b-2 transition-colors flex items-center gap-1.5 cursor-pointer ${
              ...
              <button
                ...
                className={`text-xs font-bold pb-1 border-b-2 transition-colors cursor-pointer ${
                  activeTab === "questions"
                    ? "border-purple-600 text-purple-700 dark:text-purple-400"
                    : "border-transparent text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200"
                }`}
          ...
          {activeTab === "optimization" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">AEO Optimization Actions</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Prioritized tasks to boost AI answer visibility and citations</p>
                </div>
                <Link
                  href="/aeo/actions"
              ...
              {recommendations.length === 0 ? (
                <div className="py-12 text-center space-y-2">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">No optimization recommendations recorded yet.</p>
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {recommendations.map((rec) => (
                    <div key={rec.id} className="py-3.5 flex items-center justify-between gap-4 hover:bg-purple-50/20 dark:hover:bg-purple-950/20 px-2 rounded-xl transition">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                              rec.priority === "critical"
                                ...
                                : rec.priority === "high"
                                ? "bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border dark:border-amber-800/60"
                                : "bg-purple-100 dark:bg-purple-950/60 text-purple-800 dark:text-purple-300 border dark:border-purple-800/60"
                            }`}
                          >
                            {rec.priority}
                          </span>
                          <span className="text-xs font-bold text-slate-900 dark:text-white truncate">{rec.title}</span>
                          <span className="text-[10px] text-slate-400">• {rec.category}</span>
                        </div>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-1">{rec.reason}</p>
                      </div>

                      <div className="flex items-center gap-3 flex-shrink-0">
                        <span className="text-xs font-bold text-purple-700 dark:text-purple-400">+{rec.estimated_impact || rec.opportunity_score} pts</span>
                        <span
                          className={`px-2 py-0.5 text-[10px] font-semibold rounded-full capitalize ${
                            rec.status === "fixed" || rec.status === "implemented"
                              ...
                              ? "bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300 border dark:border-blue-800/60"
                              : "bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 border dark:border-purple-800/60"
                          }`}
                        >
                          {rec.status}
                        </span>
          ...
          {activeTab === "questions" && (
            <div className="space-y-3">
              {questions.length === 0 ? (
                <div className="py-12 text-center space-y-2">
                  <p className="text-xs text-slate-500 dark:text-slate-400">No prompts tracked yet for this project.</p>
                    ...
                    <div key={q.id} className="py-3 flex items-center justify-between gap-4">
                      <div className="space-y-1 min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-500 dark:text-slate-400">
                          <span className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium">{q.category}</span>
            ...
            <div className="space-y-3">
              {entities.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {entities.map((e) => (
                    <div key={e.id} className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{e.entity_name}</span>
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-purple-100 dark:bg-purple-950/60 text-purple-800 dark:text-purple-300 border dark:border-purple-800/60">
                          {e.entity_type}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 pt-1 border-t border-slate-100 dark:border-slate-800">
                        <span>Mentions: {e.mentions_count}</span>
                        <span className="text-purple-700 dark:text-purple-400 font-bold">{e.visibility_rate}% Vis</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {citations.map((c) => (
                    <div key={c.id} className="py-3 flex items-center justify-between gap-4">
                      ...
                      <span className="px-2 py-0.5 text-[10px] font-semibold rounded bg-purple-50 dark:bg-purple-950/60 text-purple-800 dark:text-purple-300 border dark:border-purple-800/60 uppercase">
                        {c.engine}
                      </span>
                    </div>
                  ))}
          ...
          {activeTab === "monitoring" && (
            <div className="space-y-6">
              ...
              <div className="p-5 rounded-xl bg-purple-50/50 dark:bg-purple-950/30 border border-purple-100 dark:border-purple-800/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-bold text-purple-950 dark:text-purple-200">Monitoring Engine Schedule</h3>
                    <span
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a]">
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Monitoring Health</span>
                ...
                <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a]">
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Brand Share of Voice</span>
                  <p className="text-2xl font-black text-purple-950 dark:text-purple-100 mt-1">
                    {executiveIntel ? `${executiveIntel.competitive_position.brand_share_of_voice.toFixed(1)}%` : "--"}
                  </p>
                  <Link href="/aeo/competitors" className="text-[11px] text-purple-600 dark:text-purple-400 hover:underline mt-1 block">
                    View Competitor Breakdown &rarr;
                  </Link>
                </div>

                <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a]">
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Data Freshness</span>
                  <p className="text-2xl font-black text-purple-950 dark:text-purple-100 mt-1">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/aeo/alerts" className="text-xs text-purple-600 dark:text-purple-400 hover:underline">
                    View All Alerts
                  </Link>
                </div>

                {projectAlerts.length === 0 ? (
                  <p className="text-xs text-slate-400 py-4 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-lg bg-slate-50 dark:bg-slate-900/40">
                  ...
                  <div className="space-y-2">
                    {projectAlerts.map((alt) => (
                      <div
                        key={alt.id}
                        className="p-3 rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between gap-3 text-xs"
                      >
                        <div>
                          <span className="font-semibold text-slate-800 dark:text-slate-200">{alt.title}</span>
                          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{alt.description}</p>
                        </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/aeo/changes" className="text-xs text-purple-600 dark:text-purple-400 hover:underline">
                    View Change Center
                  </Link>
                </div>
                ...
                {projectChanges.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {projectChanges.map((chg) => (
                      ...
                      >
                        <div>
                          <span className="font-semibold text-slate-800 dark:text-slate-200">{chg.description}</span>
                          <span className="text-[10px] text-slate-400 block mt-0.5">
                            {new Date(chg.created_at).toLocaleString()}
                          </span>
                        </div>
                        {chg.delta !== null && chg.delta !== undefined && (
                          <span className={`font-bold ${chg.delta > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>
        ...
        {isAddQuestionOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <div className="bg-white dark:bg-[#0f172a] rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-base font-bold text-slate-900 dark:text-white">Add Search Prompt</h3>
                <button onClick={() => setIsAddQuestionOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateQuestion} className="space-y-3.5">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Question / Prompt</label>
                  <textarea
