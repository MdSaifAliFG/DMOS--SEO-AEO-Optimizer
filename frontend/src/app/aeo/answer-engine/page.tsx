      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 shadow-xs">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Answer Engine Synthesis & Workspace</h2>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {engines.map((engine) => {
            ...
            return (
              <Card key={engine.engine_id} className="p-5 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] space-y-3.5">
                <div className="flex items-start justify-between">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className={`w-2.5 h-2.5 rounded-full ${isConnected ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"}`} />
                      <h3 className="text-sm font-bold text-slate-900 dark:text-white">{engine.name}</h3>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-center font-mono">
                  <div className="p-2 bg-slate-50 dark:bg-slate-900/60 rounded-lg border border-slate-100 dark:border-slate-800">
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block uppercase font-sans">Answers</span>
                    <span className="text-xs font-bold text-slate-900 dark:text-white">{engine.tracked_questions}</span>
                  </div>
                  <div className="p-2 bg-purple-50 dark:bg-purple-950/40 rounded-lg border border-purple-100 dark:border-purple-800/60">
                    <span className="text-[10px] text-purple-700 dark:text-purple-300 block uppercase font-sans">Brand Vis</span>
        ...
        <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0f172a] space-y-4 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
            <div className="space-y-1">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Collected Answer Responses ({filteredAnswers.length})</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Live AI search outputs collected during project analysis runs.</p>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 w-44"
                />
              </div>
          ...
          {isLoading ? (
            ...
            <div className="py-16 text-center space-y-2">
              ...
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">No AI answers collected yet</p>
            ...
            <div className="space-y-4">
              {filteredAnswers.map((ans) => (
                <div
                  key={ans.id}
                  className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-purple-300 dark:hover:border-purple-600/60 bg-slate-50/40 dark:bg-slate-900/40 space-y-3 transition-colors"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      {ans.question_text && (
                        <p className="text-xs font-bold text-slate-900 dark:text-white">"{ans.question_text}"</p>
                      )}
                      <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                        <span className="px-2 py-0.5 rounded font-bold uppercase text-[10px] bg-purple-100 dark:bg-purple-950 text-purple-800 dark:text-purple-300">
                          {ans.engine}
                        </span>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {ans.brand_mentioned ? (
                        <span className="px-2.5 py-1 text-[11px] font-bold rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60">
                          Brand Mentioned {ans.brand_position ? `(Rank #${ans.brand_position})` : ""}
                        </span>
                      ) : (
                        <span className="px-2.5 py-1 text-[11px] font-medium rounded-full bg-slate-200/80 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                          Not Mentioned
                        </span>
                      )}
                    </div>
                  ...
                  {/* Answer Preview */}
                  <p className="text-xs text-slate-700 dark:text-slate-200 leading-relaxed bg-white dark:bg-[#0f172a] p-3.5 rounded-xl border border-slate-100 dark:border-slate-800 whitespace-pre-wrap line-clamp-4">
                    {ans.answer_text}
                  </p>
                  ...
                  <div className="flex items-center justify-between pt-1 text-xs">
                    <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 text-[11px]">
                      {ans.citations_count > 0 && (
                        <span className="flex items-center gap-1 font-medium text-purple-700 dark:text-purple-400">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded flex items-center gap-1 text-[11px]"
                      >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <div className="bg-white dark:bg-[#0f172a] rounded-2xl p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <button onClick={() => setExpandedAnswer(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                  <X className="w-5 h-5" />
                </button>
              </div>

              {expandedAnswer.mention_snippets && expandedAnswer.mention_snippets.length > 0 && (
                <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-xs text-emerald-950 dark:text-emerald-200 space-y-1">
              ...
              <div className="space-y-1">
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Full Generated Answer Text</span>
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-200 whitespace-pre-wrap leading-relaxed max-h-72 overflow-y-auto">
