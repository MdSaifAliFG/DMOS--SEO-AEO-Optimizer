...
export default function AeoAlertsPage() {
  ...
  const getSeverityBadge = (severity: string) => {
    switch (severity.toLowerCase()) {
      case "critical":
        return (
          <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/60 flex items-center gap-1">
            <AlertTriangle className="w-3 h-3 text-rose-600 dark:text-rose-400" /> Critical
          </span>
        );
      case "high":
        return (
          <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60 flex items-center gap-1">
            <AlertTriangle className="w-3 h-3 text-amber-600 dark:text-amber-400" /> High
          </span>
        );
      case "medium":
        return (
      ...
      <div className="space-y-6">
        ...
        {isLoading ? (
          <div className="p-16 flex flex-col items-center justify-center text-center bg-white dark:bg-[#0f172a] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
            ...
            <p className="text-slate-800 dark:text-slate-200 font-semibold text-sm">Scanning Alert Records...</p>
          </div>
        ) : projects.length === 0 ? (
          ...
          <Card className="bg-white dark:bg-[#0f172a] border-slate-200/80 dark:border-slate-800 shadow-xs rounded-2xl p-12 text-center">
            ...
            <h3 className="text-base font-bold text-slate-900 dark:text-white">No Alerts Found</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto mt-1">
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="space-y-1.5 flex-1">
                    ...
                    <h3 className="text-base font-bold text-slate-900 dark:text-white pt-1">{alert.title}</h3>
                    ...
                    <div className="flex flex-wrap items-center gap-4 pt-2 text-xs text-slate-500 dark:text-slate-400">
                      <span>Status: <strong className="text-slate-800 dark:text-slate-200 capitalize font-bold">{alert.status}</strong></span>
                    ...
                    {alert.status === "acknowledged" && (
                      <>
                        ...
                        <Button
                          ...
                          disabled={actionInProgressId === alert.id}
                          className="w-full border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center justify-center gap-1 rounded-xl"
                        >
