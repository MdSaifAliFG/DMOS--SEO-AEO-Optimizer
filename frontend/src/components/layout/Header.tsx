...
import { HealthResponse, BillingSummary } from "@/lib/types";
...
export const Header: React.FC<HeaderProps> = ({ onOpenMobileNav }) => {
  ...
  const fetchBilling = React.useCallback(() => {
    api
      ...
      .then((data) => {
        if (data) {
          const currentKey = user?.email || user?.id || "guest";
          globalBillingCacheByUser[currentKey] = data;
  ...
  useEffect(() => {
    let isMounted = true;
    if (!globalHealthCache) {
      api
        .getHealth()
        .then((data) => {
          if (isMounted) {
            globalHealthCache = data;
            setHealth(data);
          }
        })
        .catch(() => {
  ...
  const getPageHeaderInfo = () => {
    ...
    if (pathname === "/billing") {
      return {
        ...
        subtitle: "Manage your subscription plan, credit balance, and Razorpay top-ups",
      };
    }
    if (pathname === "/billing/usage") {
      return {
        title: "Credit Usage & Analytics",
        subtitle: "Detailed consumption breakdown across SEO, AEO, and GEO engines",
      };
    }
    if (pathname === "/billing/history") {
      return {
        title: "Credit Ledger & Transactions",
        subtitle: "Immutable audit trail of credit allocations, consumptions, and purchases",
      };
    }
    if (pathname === "/billing/invoices") {
      return {
        title: "Invoices & Payment Receipts",
        subtitle: "Review payment records, transaction history, and download PDF receipts",
      };
    }
    if (pathname === "/billing/success") {
      return {
        title: "Payment Confirmed",
        subtitle: "Your subscription or credit top-up has been successfully activated",
      };
    }
    if (pathname === "/billing/cancelled") {
      return {
        title: "Checkout Cancelled",
        subtitle: "No charges were made to your account",
      };
    }
