      ...
      <div className="rounded-2xl bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs">
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto flex-wrap justify-between sm:justify-end">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400">
              ...
              <select
                ...
                className="px-2.5 sm:px-3 py-2 bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 transition-all shadow-2xs cursor-pointer font-medium"
              >
          ...
          <table className="w-full text-left text-xs min-w-[720px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="py-12 text-center text-slate-400">
                    <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">No crawled pages found</p>
                    <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">Try resetting search filters or check status filters.</p>
                  </td>
                </tr>
              ) : (
                pages.map((page) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4 max-w-md">
                      <div className="font-mono text-emerald-700 dark:text-emerald-400 font-medium text-xs truncate">
                        {page.url}
                      </div>
                      <div className="text-slate-500 dark:text-slate-400 text-[11px] truncate mt-0.5">
                        {page.title || <span className="italic text-slate-400 dark:text-slate-500">No title found</span>}
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-center">
                      {(page.issues_count ?? 0) > 0 ? (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-800/60">
                          {page.issues_count}
                        </span>
                      ) : (
        ...
        <div className="p-3.5 sm:p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/60 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-600 dark:text-slate-400">
          <div className="text-center sm:text-left">
            ...
            <span className="font-bold text-slate-900 dark:text-white">{total}</span> page{total === 1 ? "" : "s"}
          </div>
